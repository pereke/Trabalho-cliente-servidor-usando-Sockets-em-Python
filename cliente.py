...
#fazer a parte dos parametros aqui (verificar se vai querer fazer o ping)
sock_tcp.connect(destino_tcp)
mensagem_inicial = 'fazer ping' 
sock_tcp.send(mensagem_inicial.encode())
...
for i in range(pacotesEnviar):
    ...
    try:
        resposta, server = sock_udp.recvfrom(1024)
...
mensagem_fim_ping = 'fim ping' 
sock_tcp.send(mensagem_fim_ping.encode())

mensagem_fim = 'fim' 
sock_tcp.send(mensagem_fim.encode())
